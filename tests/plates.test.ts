...
test('should create a new plate on diet', async ({ page }) => {
  await page.goto('/', {
	...
	await page.getByText('Cadastrar refeição').click()

	await expect(page.getByText('Refeição criada com sucesso!')).toBeVisible()
...
test('should edit a plate', async ({ page }) => {
  await page.goto('/', {
	...
	await expect(page.getByText('Refeição atualizada com sucesso!')).toBeVisible()
})
...
test('should delete a plate', async ({ page }) => {
  await page.goto('/', {
	...
	await expect(page.getByText('Refeição excluída com sucesso!')).toBeVisible()
})
